...
keyauthapp = api("Shiba Builder", "SW7eOCy9S2", "af78ae435920d79de833f8415634f932fb29c5d94fff8a3cd928a84988b49db6","1.0")

print("Connecting to Skidders Server")
print("Connected!")
keyauthapp.init()
...
1.Login
2.Register User
3.Login/Register with only Serial Key
""")
...
if ans=="1": 
    ...
    user = input('Provide UserName: ')
    password = input('Provide PassWord: ')
    license = input('Provide Serial Key: ')
    keyauthapp.register(user,password,license) 
elif ans=="3":
    key = input('Enter your Serial Key: ')
...
#         time.sleep(0.05)
#         if elapsed == '00:01:00':
#             print(f"{Fore.GREEN}[!]{Style.RESET_ALL} Timer finished.")
#             break
...
class DiscordGen:
    ...
    def register(self):
        ...
        try: #if date could not be found via divs
            ...
            actions.send_keys(str(random.randint(1,28))) # Submitting the month

            actions.send_keys(Keys.ENTER)


            actions.send_keys(str(random.randint(1,12))) # Submitting the day
            ...
            free_print(f"\n{Fore.GREEN}|!|{Style.RESET_ALL} " + 'Error in typing date. You need to type the date manually.')
            input(f"{Fore.GREEN}|!|{Style.RESET_ALL}Have you put the date? [y | n] > ") # Fixed typo

        free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}You must submit the form')
        ...
        if dateWorking:
            ...
            try: 
                ...
                free_print(f"{Fore.GREEN}|*|{Style.RESET_ALL}Button undetected. Ignoring..")
                pass
        ...
        while True:
            lock.acquire()
            checker = input(f"{Fore.GREEN}|!|{Style.RESET_ALL} Have you solved the captcha and submit? [y | n] > ")
            lock.release()
            if checker == "y":
    ...
    def verify_account(self,link):
        ...
        free_print(f"{Fore.GREEN}|!|{Style.RESET_ALL}Email verified.")

    def close_driver(self):
        self.driver.close()

def start_verify(email, email_type):  #email, 'dot'/'plus'
    free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}Verifying the email...')
    raw_email = email
        ...
        for x in range(len(gmailnator_inbox)):  # for each email
            ...
            if 'Discord' in discord_keywords:
                ...
                try:
                    ...
                    free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}Verify link extracted.')
                except IndexError:
                    free_print(f'{Fore.GREEN}|!|{Style.RESET_ALL}List index out of range.')
                    discord_verify = None

                return discord_verify

            else:
                free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}Discord keyword not found there. Trying another one...')
        free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}Still no mail from discord. Retry count: {retry_count}')
        free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL}we will try again in 10 seconds. Waiting for Discord email.')
        time.sleep(10)
    free_print(f'{Fore.GREEN}|*|{Style.RESET_ALL} Discord keyword not found. Unable to verify account via email.')
    return False  # cant find any email with the word discord in it

def worker(proxy=None):
    if proxy:
        print(f"{Fore.GREEN}|*|{Style.RESET_ALL}Proxy used {proxy} ")
    free_print(f"{Fore.GREEN}|!|{Style.RESET_ALL}Scraping email. ")

    g = GmailnatorGet()
    new_email = g.get_email()
    
    free_print(f"{Fore.GREEN}|*|{Style.RESET_ALL}Email scraped : {new_email}")
    ...
    try:
        ...
        token = str(d.token)
        lock.acquire()
        with open('output/tokens.txt', 'a', encoding='UTF-8') as login_file:
        ...
        free_print(f"{Fore.GREEN}[!]{Style.RESET_ALL}Webdriver Error. impossible to continue.")

    free_print(f"{Fore.GREEN}[!]{Style.RESET_ALL}Task completed")
...
def menu():
    ...
    if len(proxies) != 0:
        os.system('title IntruserGenerator || Discord account generator || Proxies detected')

    else:
        os.system('title IntruserGenerator || Discord account generator ')
    UI.banner()
    UI.start_menu()

    try:
        user_input = int(input(f"{Fore.GREEN}|$|>{Style.RESET_ALL} "))
        print('\n\n')
    except ValueError:
        user_input = 0
...
def main():
    ...
    if continue_program:
        if user_thread:
            print(f"{Fore.GREEN}[WARNING]{Style.RESET_ALL} Use threads at your own risk, 3 is a decent number of threads (Chrome windows).")
            num_thread = int(input(f"{Fore.GREEN}|>|{Style.RESET_ALL} Enter number of threads> "))
        ...
        if user_thread:
            ...
            if len(proxies) != 0:
                os.system('title IntruserGenerator ^|| Proxy: True ^|| Threading: True')
                ...
                os.system('title IntruserGenerator ^|| Proxy: False ^|| Threading: True')
            ...
            if len(proxies) != 0:
                os.system('title IntruserGenerator ^|| Proxy: True ^|| Threading: False')
                worker(random.choice(proxies))

            else:
                os.system('title IntruserGenerator ^|| Proxy: False ^|| Threading: False')
                worker()    
